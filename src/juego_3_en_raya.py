    ...
    * movimientos_jugador: dict con el conjunto de movimientos de un
    ...
    for fila in movimientos_jugador:
        ...
        if len(movimientos_columna)==3:
            return True
    columnas = {}
    for fila, columnas_ocupadas in movimientos_jugador.items():
        for col in columnas_ocupadas:
            if col not in columnas:
                columnas[col] = 0
            columnas[col] += 1

    if any(count == n for count in columnas.values()):  # Si hay n fichas en una columna
        return True

    if all(i in movimientos_jugador and i in movimientos_jugador[i] for i in range(n)):
        return True
    if all(i in movimientos_jugador and (n - 1 - i) in movimientos_jugador[i] for i in range(n)):
        return True
    return False
